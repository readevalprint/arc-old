...
(def parseform (raw sep)
  (withs (seplines (erp:systr "grep -na ^`head -n 1 \""raw"\" | head -c -2` \""raw"\"")
          seplist (erp:map [tokens _ #\:] (tokens seplines)))
      (getformdata raw seplist)))

(def getformdata (raw seplist) 
  (withs (start (+ 1 (coerce (caar seplist) 'int ))
          end (- (coerce (car:cadr seplist) 'int ) 1)
          filename (erp:form-file-name raw start)
          inputname (erp:form-input-name raw start))
    (if filename
...
(def form-file-contents (raw start end filename)
  (let out (string uploaddir* (rand-string 10) "_" filename)
